...
from peft import get_peft_model,LoraConfig,PeftModelForCausalLM,prepare_model_for_kbit_training,prepare_model_for_kbit_training
import jsonlines,unicodecsv,datasets
import os,glob
from modeling_moe_llama import LlamaMoEForCausalLM
from dotenv import load_dotenv
load_dotenv()
model_name="./moe-merged"
runname="moe-finish-f1"
...
model = LlamaMoEForCausalLM.from_pretrained(model_name,trust_remote_code=True,torch_dtype=torch.float16)
...
print('unfreezing target modules')
model.enable_input_require_grads()

for layer in model.model.layers:
    # unfreeze gate
    layer.gate.to(torch.float32)
    layer.gate.requires_grad_(True)
    # # unfreeze experts 1
    # layer.experts[0].requires_grad_(False)
    # layer.experts[1].requires_grad_(True)
...
def Tokenize(ex):
    input_ids= tokenizer(ex["text"],text_target=ex["text"], padding=False, truncation=True, max_length=2048).input_ids
...
ds=datasets.load_dataset("mmnga/wikipedia-ja-20230720-1k",split="train")


tokenized_dataset=datasets.concatenate_datasets([ds.map(Tokenize,batched=False,batch_size=1,num_proc=4),
                                                 noja_map.map(tokenize_function,batched=False,batch_size=1,num_proc=4)]).select(range(1000))
tokenized_datasets=tokenized_dataset.train_test_split(test_size=0.05)
