...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMoEConfig, layer_idx: int):
        ...
        self.num_experts_per_token = config.num_expert_per_tok
    ...
    def forward(
        ...
        **kwargs,
        ...
        y = torch.empty_like(x).to(x.dtype)
        for i, expert in enumerate(self.experts):
            y[flat_expert_indices == i] = expert(x[flat_expert_indices == i]).to(x.dtype)
        y = (y.view(*expert_weights.shape, -1) * expert_weights.unsqueeze(-1)).sum(dim=1)
        hidden_states= y.view(*orig_shape)
        hidden_states = residual + hidden_states
